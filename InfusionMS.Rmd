---
title: "Assembling simulation results for ms"
output: 
  html_document:
    toc: true
    toc_depth: 3
...
correctdir <- "D:/home/francois/travail/stats/Infusionplus"
...
checkwd()
source("diy2inf/package/R/performance_fns.R")
source("diy2inf/package/R/output_files_manips.R")
MSdir <- "MS/"
bootCI <- "BartCI"
```
```{r knitr_utilities}
# Utility to make sure necessary files are copied in a dedicated directory
load4MS <- function(file, ...) {
  copyforMS <- paste0("MS/",file)
...
```{r logLexploration, fig.width = 4, fig.height = 4}
modelpath_nickname <- "N_7from17"
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/saves4Rmd.rda", verbose=TRUE)
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/saved_fits_2.1.187.1/upsliks_1.rda", verbose=TRUE)
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/reftable_abcrf.rda")
...
```
but the long part of the computation was saved so that 
```{r prof2D_b3saved, fig.width = 6, fig.height = 6}
load4MS(file="diy2inf_simuls/admixtOutOfA/P_4from17/saved_info_for_2Dprofile/slik.22K.rda")
load4MS(file="diy2inf_simuls/admixtOutOfA/P_4from17/saved_info_for_2Dprofile/profiles_info.rda")
pdf("MS/2Dprofile_b3_P4from17.22K.pdf",width = 5,height = 6.5) # not controlled by the chunk args args...
