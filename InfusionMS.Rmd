...
The following chunk of code makes sure that an `R` script used to assemble the `SNLE_summaries.rda` file from the raw Python output files is included:
```{r save_scripts_SNLE_8p}
save4MS("diy2inf_simuls/Harmonia/D_axy_8pars/SNLE/summarize_snle.R")  
```
An `SNLE/allfilesforeachjob/` subdirectory should also be present, including all files used to run the simulations.
...
```
### Inference by SNLE using `sbi` python package
The following chunk of code makes sure that the `python` script used to run the simulations, and an `R` script used to assemble the `SNLE_summaries.rda` file from the raw Python output files, are included in the published code:
```{r save_scripts_MVN}
save4MS("toyTests/MVNcovmat/SNLE/snle_MVNcovmat.py") 
