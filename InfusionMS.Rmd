---
title: "Assembling figures, tables and SI for summary-likelihood ms"
output: 
  html_document:
    toc: true
    toc_depth: 3
...
```{r knitr_utilities}
# Utilities to make sure necessary files are copied in the published code:
save4MS <- function(file, SIdir="./") {
...
load4MS <- function(file, SIdir="./", ...) {
  save4MS(file=file, SIdir=SIdir)
  load(file, ..., envir=knitr::knit_global())
}

read4MS <- function(file, SIdir="./") {
  save4MS(file=file, SIdir=SIdir)
  read.table(file,dec='.',sep=';')
}
...
vec2tex  <- function(locv, retained, bias_pos, RMSE_pos, 
                     ...
                     loc_ord_names) {
  locv <- locv[retained]
  if (missing(loc_ord_names)) {
    locv <- locv[intersect(ord_names,names(locv))] ## using global 'ord_names' variable
  } else locv <- locv[intersect(loc_ord_names,names(locv))]
  minBias <- bias_pos[which.min(abs(locv[bias_pos]))]
  minRMSE <- RMSE_pos[which.min(abs(locv[RMSE_pos]))]
  oldopt <- options(scipen=666)
  char_locv <- paste0(round(locv,digits = 5),"00000")
  options(oldopt)
  char_locv[char_locv=="100000"] <- "1.00000"
  isnegvals <-  locv < 0
  for (it in seq_along(char_locv)) {
   char_locv[it]  <- substring(char_locv[it],1L, 
                               2L+max(3L, -floor(log(abs(locv[it]),10))) +isnegvals[it])
  }
  ...
  if (length(cvrg_pos)>1L) {
    absd <- abs(locv[cvrg_pos]-0.95)
    bestcvrg <- cvrg_pos[which(absd<min(absd)+1e-4)]
...
perf_summs <- cp_perf_summs <- ad_hoc_fn(ecdfs_df, DGparlist = scaDGparlist, plot.=plot.)
...
### Compare inferences of $\log(N_4\bar{\mu})$
```{r D8Th4pars}
...
load4MS("diy2inf_simuls/Harmonia/D_axy_8pars_logTh4/S_obs_table.rda", verbose=TRUE) # for S_obs_table_info
  ...
  tib <- data.frame(ABCRF=df_extract(ecdfs_df,c("abcrf","logTh4","expectation"))[,1],
                    MSLE=df_extract(ecdfs_df,c("MSL","MSLE","logTh4")))
...
### Inference by SNLE using `sbi` python package
The following chunk of code makes sure that all files is included:
```{r save_files_SNLE_8p}
save4MS("diy2inf_simuls/Harmonia/D_axy_8pars/SNLE/summarize_snle.R")  
filenames <- c("data_HA_INFUSION.txt", "diyabc-RF-linux-v1.1.36-o", "headerRF.txt",             
"RNG_state_0000.bin", "S_obs_table.txt", "snle_D_axy_8pars.py", "statobsRF.txt" )
for (nam in filenames) save4MS(paste0("diy2inf_simuls/Harmonia/D_axy_8pars/SNLE/allfilesforeachjob/", nam))
```
...
load4MS("diy2inf_simuls/Harmonia/D_axy_8pars/SNLE/SNLE_summaries.rda", verbose=TRUE) # postm, q1s,s2s

summs <- vector("list", 8L)
names(summs) <- parms
for (parm in parms) {
  m <- (postm[,parm] - scaDGP[parm])/(UPPER[parm] - LOWER[parm])
  cover <- (q1s[,parm]< scaDGP[parm] & q2s[,parm] > scaDGP[parm] )
  summs[[parm]] <- c(cp_perf_summs[[parm]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI","cvrg.rawbootP","cvrg.bartlettP")],
                     c(bias.SNLE=mean(m),
                          RMSE.SNLE=sqrt(mean(m^2)), 
                          cvrg.SNLE=sum(cover)/length(cover))
  )
}

texcode <- lapply(
  summs, vec2tex, 
  retained=names(summs[[1]]),
  bias_pos=c(1,6), RMSE_pos=c(2,7), # cvrg_pos=c(3,6),
  loc_ord_names=names(summs[[1]]))
texcode <- unlist(texcode)
partexcode <- sapply(names(texcode), par2ltx )
texcode <- paste(partexcode,"&", texcode)
writeLines(texcode, con=paste0(MSdir,"table", modelpath_nickname,"SNLE.ltx"))
...
### $d_3/N_{\textrm{bn34}}$ at its boundaries

```{r ST7from17, message=FALSE}
load4MS("diy2inf_simuls/admixtOutOfA/T_7from17_Nbn34_low/saves4Rmd.rda", verbose=TRUE)
parms <- names(scaDGP <- unlist(scaDGparlist))
load4MS("diy2inf_simuls/admixtOutOfA/T_7from17_Nbn34_low/summaries_list.1_400.v2.2.20.rda") 
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df <- get(full_df_name)
perf_summs_low <- ad_hoc_fn(ecdfs_df, DGparlist = scaDGparlist, plot.=plot.)
#
load4MS("diy2inf_simuls/admixtOutOfA/S_7from17_Nbn34_hi/saves4Rmd.rda", verbose=TRUE)
parms <- names(scaDGP <- unlist(scaDGparlist))
load4MS("diy2inf_simuls/admixtOutOfA/S_7from17_Nbn34_hi/summaries_list.1_400.v2.2.20.rda") 
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df <- get(full_df_name)
perf_summs_hi <- ad_hoc_fn(ecdfs_df, DGparlist = scaDGparlist, plot.=plot.)
#
```
#### Table
```{r TableST7bnd34}
{
  texcode <- lapply(perf_summs_low, vec2tex, retained=inms,
                    bias_pos=bias_pos, RMSE_pos=RMSE_pos)
  texcode <- unlist(texcode)
  partexcode <- sapply(names(texcode), par2ltx )
  texcode <- paste(partexcode,"&", texcode)
  writeLines(texcode, con=paste0(MSdir,"tableT_7from17_Nbn34_bnds_low.ltx"))
  #
  texcode <- lapply(perf_summs_hi, vec2tex, retained=inms,
                    bias_pos=bias_pos, RMSE_pos=RMSE_pos)
  texcode <- unlist(texcode)
  partexcode <- sapply(names(texcode), par2ltx )
  texcode <- paste(partexcode,"&", texcode)
  writeLines(texcode, con=paste0(MSdir,"tableS_7from17_Nbn34_bnds_hi.ltx"))
}
```
...
```{r N7from17altsampling, message=FALSE}
modelpath_nickname <- "N_7from17"
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/saves4Rmd.rda", verbose=TRUE)
parms <- names(scaDGP <- unlist(scaDGparlist))
...
To perform these analyses, make a local modification of the `max_table_size` in 
the `generic_workflow.R` script.
```{r N7from17samplings6K, message=FALSE}
modelpath_nickname <- "N_7from17"
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/saves4Rmd.rda", verbose=TRUE)
parms <- names(scaDGP <- unlist(scaDGparlist))
...
### Inference by SNLE using `sbi` python package
The following chunk of code makes sure that all files is included:
```{r save_scripts_SNLE_7p}
save4MS("diy2inf_simuls/admixtOutOfA/N_7from17/SNLE/summarize_snle.R")  
filenames <- c("diyabc-RF-linux-v1.1.36-o", "headerRF.txt", "human.snp", "human.snpbin.txt",
               "maf.txt",  "S_obs_table.txt", "RNG_state_0000.bin",  
               "snle_N_7from17.py", "statobs.txt","statobsRF.txt"  )
for (nam in filenames) save4MS(paste0("diy2inf_simuls/admixtOutOfA/N_7from17/SNLE/allfilesforeachjob/", nam))
```
...
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/saves4Rmd.rda", verbose=TRUE)
parms <- names(scaDGP <- unlist(scaDGparlist))
load4MS("diy2inf_simuls/admixtOutOfA/N_7from17/SNLE/SNLE_summaries.rda", verbose=TRUE) # postm, q1s,s2s

summs <- vector("list", 7L)
names(summs) <- parms
for (parm in parms) {
  m <- (postm[,parm] - scaDGP[parm])/(UPPER[parm] - LOWER[parm])
  cover <- (q1s[,parm]< scaDGP[parm] & q2s[,parm] > scaDGP[parm] )
...
#### Real-data inferences

Production of SI table from saved `abcrf` and `Infusion` inference results: 

```{r realdata013from17, eval=FALSE}
load4MS("diy2inf_simuls/admixtOutOfA/O_13from17/actual_data_inferences.summaries.rda")
  
confints <- lapply(summ_inference_maxK[["CIs"]], function(v) v[[1]]$interval)
post_quantiles <- lapply(abcrf_resu, function(v) v$quantiles)
HPDints <- lapply(abcrf_resu, function(v) v$HPDint_d)
texcode <- sapply(names(confints), function(v) 
  paste(par2ltx(v), "&",
        signif(summ_inference_maxK$MSL$MSLE[[v]], 3),"&",    
        signif(abcrf_resu[[v]]$"expectation", 3),"&",    
        signif(abcrf_resu[[v]]$"med", 3),"&",    
        paste(signif(confints[[v]],3), collapse="--"),"&",
    paste(signif(HPDints[[v]],3), collapse="--"),"&",
    paste(signif(post_quantiles[[v]],3), collapse="--")," \\\\ "
  )
)

(texcode <- unlist(texcode))

writeLines(texcode, con=paste0(MSdir,"table.fit_real_O13from17.ltx"))
...
Infusion::plot2Dprof(profiles_info$fit_info, 
           ...
           color.palette = function(n) grDevices::hcl.colors(n=n ,palette="viridis", rev=TRUE),
           xylabs=c(d3_Nbn3=latex2exp::TeX("$b_3=d_3/N_{\\textrm{bn3}}$"),
...
```
### Inference by SNLE using `sbi` python package
The following chunk of code makes sure that the given files are included in the published code:
```{r save_scripts_MVN}
save4MS("toyTests/MVNcovmat/SNLE/S_obs_table.txt") # simulated data (from R) 
save4MS("toyTests/MVNcovmat/SNLE/snle_MVNcovmat.py") # python script to run inferences
# script to assemble SNLE_summaries.rda from the raw Python files:
save4MS("toyTests/MVNcovmat/SNLE/summarize_snle.R") 
```

```{r tableMVN_SNLE}
load4MS("toyTests/MVNcovmat/saves4Rmd.rda", verbose=TRUE) # LOWER, UPPER, cholDGPv
load4MS("toyTests/MVNcovmat/SNLE/SNLE_summaries.rda", verbose=TRUE) # postm, q1s,s2s
modelpath_nickname <- "MVNcovmat"

summs <- vector("list", 15L)
names(summs) <- Rord_names <- names(LOWER)
for (parm in names(summs)) {
  m <- (postm[,parm] - cholDGPv[parm])/(UPPER[parm] - LOWER[parm])
  cover <- (q1s[,parm]< cholDGPv[parm] & q2s[,parm] > cholDGPv[parm] )
  summs[[parm]] <- c(perf_summs_TFI[[parm]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI","cvrg.rawbootP","cvrg.bartlettP")],
                     c(bias.SNLE=mean(m),
                          RMSE.SNLE=sqrt(mean(m^2)), 
                          cvrg.SNLE=sum(cover)/length(cover))
  )
...
# Graphical comparison of interval widths
SLCIwidths <- numeric(15)
names(SLCIwidths) <- Rord_names
for (st in names(SLCIwidths)) {
  interv <- df_extract(ecdfs_df,c("CIs",st,"0.95","interval"))
  print(c(sum(is.na(interv[,1])),sum(is.na(interv[,2])))) # some intervals reach the bounds
  interv[is.na(interv[,1]),1] <-  LOWER[st]
  interv[is.na(interv[,2]),2] <-  UPPER[st]
  SLCIwidths[st] <- mean(interv[,2]-interv[,1])
}
plot(SLCIwidths, colMeans(q2s-q1s))
abline(0,1)
...
## Test of clustering controls

### Power rule $n^{0.31}$ versus alternatives

```{r MVNpow}
modelpath_nickname <- "MVNpow"
load4MS("toyTests/MVN6/saves4Rmd.rda", verbose=TRUE)
scaDGparlist <- as.list(cholDGPv)
load4MS("toyTests/MVN6/pow31/summaries_list.1_1000.v2.2.20.rda") # without bootstraps
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df_pow31 <- get(full_df_name)
#
load4MS("toyTests/MVN6/pow33/summaries_list.1_1000.v2.2.20.rda") # without bootstraps
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df_pow33 <- get(full_df_name)
#
load4MS("toyTests/MVN6/pow29/summaries_list.1_1000.v2.2.20.rda") # without bootstraps
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df_pow29 <- get(full_df_name)

```
```{r tableMVNpow}
  perf_summs_pow31 <- ad_hoc_fn(ecdfs_df_pow31, DGparlist = scaDGparlist, plot.=plot.)
  perf_summs_pow33 <- ad_hoc_fn(ecdfs_df_pow33, DGparlist = scaDGparlist, plot.=plot.)
  perf_summs_pow29 <- ad_hoc_fn(ecdfs_df_pow29, DGparlist = scaDGparlist, plot.=plot.)
  perf_summs <- perf_summs_pow29

  retained <- c("bias.MSLE","RMSE.MSLE","cvrg.mslCI",
                "bias.MSLE ","RMSE.MSLE ","cvrg.mslCI ",
                "bias.MSLE  ","RMSE.MSLE  ","cvrg.mslCI  ") # The spaces in the names matter!
  for (it in seq(6)) {
    perf_summs[[it]] <- c(perf_summs_pow29[[it]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI")],
                          perf_summs_pow31[[it]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI")],
                          perf_summs_pow33[[it]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI")]) 
    names(perf_summs[[it]]) <- retained
  }
  texcode <- lapply(perf_summs, vec2tex, retained=retained, loc_ord_names=retained,
                    bias_pos=c(1,4,7), RMSE_pos=c(2,5,8), cvrg_pos=c(3,6,9))
  texcode <- unlist(texcode)
  partexcode <- sapply(names(texcode), par2ltx )
  texcode <- paste(partexcode,"&", texcode)
  writeLines(texcode, con=paste0(MSdir,"table",modelpath_nickname,".ltx"))
...
### $P\leq n/4$ rule versus alternatives

```{r MVNnr3nr8}
modelpath_nickname <- "MVNnr3nr8"
load4MS("toyTests/MVNcovmat/saves4Rmd.rda", verbose=TRUE)
scaDGparlist <- as.list(cholDGPv)
load4MS("toyTests/nr8/MVNcovmat/summaries_list.1_400.v2.2.20.rda") # without bootstraps
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df_nr8 <- get(full_df_name)
load4MS("toyTests/nr3/MVNcovmat/summaries_list.1_200.v2.2.20.rda") # with bootstraps
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df_nr3a <- get(full_df_name)
# remove bootstraps
for (it in seq(200)) {
  for (jt in seq(15)) {
  ecdfs_df_nr3a[it,"CIs"][[1]][[jt]][["0.95"]]  <-   
    ecdfs_df_nr3a[it,"CIs"][[1]][[jt]][["0.95"]][c("lowerpar","upperpar","interval")]
  }
}
load4MS("toyTests/nr3/MVNcovmat/summaries_list.201_400.v2.2.20.rda") # without bootstraps
lnames <- names(summaries_list)
(full_df_name <- paste0("df_",lnames[1],"_",tail(lnames,1)))
assign(full_df_name, do.call(rbind, summaries_list))
ecdfs_df_nr3b <- get(full_df_name)
ecdfs_df_nr3 <- rbind(ecdfs_df_nr3a,ecdfs_df_nr3b)

```
```{r tableMVNnr3nr8}
  perf_summs_nr8 <- ad_hoc_fn(ecdfs_df_nr8, DGparlist = scaDGparlist, plot.=plot.)
  perf_summs_nr3 <- ad_hoc_fn(ecdfs_df_nr3, DGparlist = scaDGparlist, plot.=plot.)
  perf_summs <- perf_summs_nr3
  retained <- c("bias.MSLE","RMSE.MSLE","cvrg.mslCI",
                                 "bias.MSLE ","RMSE.MSLE ","cvrg.mslCI ")
  for (it in seq(15)) {
    perf_summs[[it]] <- c(perf_summs_nr3[[it]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI")],
                          perf_summs_nr8[[it]][c("bias.MSLE","RMSE.MSLE","cvrg.mslCI")]) 
    names(perf_summs[[it]]) <- retained
  }
  texcode <- lapply(perf_summs, vec2tex, retained=retained, loc_ord_names=retained,
                    bias_pos=c(1,4), RMSE_pos=c(2,5), cvrg_pos=c(3,6))
  texcode <- unlist(texcode)
  partexcode <- sapply(names(texcode), par2ltx )
  texcode <- paste(partexcode,"&", texcode)
  writeLines(texcode, con=paste0(MSdir,"table",modelpath_nickname,".ltx"))
